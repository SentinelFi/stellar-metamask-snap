...
8. **Publish by pushing the tag.** Publishing runs in CI, not from a laptop:
   ...
   - **`publish`** (the only job with `id-token: write`, inside the `npm-publish` environment) never checks out the repository. It downloads the tarballs, refuses them unless their digests equal the ones the seal job recorded, and runs `npm publish <tarball> --provenance`, snap first and connector second (the connector pins the snap version it installs, so the reverse order leaves a window in which a dapp can install a broken pair). Authentication is npm trusted publishing (OIDC); no npm token exists in the repository's secrets once the one-time setup below is done.
     ...
     Publication is also ordered and resumable. The job runs under a fixed concurrency group shared by every release tag, so two releases can never publish at once and a queued one waits rather than being cancelled. Before publishing, it reads each package's registry `latest` and refuses any publish that would move the tag backward (npm assigns every publish to `latest`, so a slower run for an older tag finishing after a newer release would otherwise repoint fresh installs at the superseded pair). A version already on the registry is accepted only when its integrity equals the sealed tarball's, in which case its publish is skipped: that is what lets a release that published the snap but failed before the connector be finished by re-running the workflow, since npm versions are immutable and a blind re-publish would fail on the half that landed. A version that exists with different bytes fails the release outright. After both publishes, the job verifies against the registry that both versions serve exactly the sealed bytes and that `latest` resolves to them for both packages. A staging-tag publish promoted to `latest` afterwards would be stronger still, but npm trusted publishing authenticates `npm publish` only: moving dist-tags would require a long-lived npm token in this repository, which is the standing credential this workflow exists to avoid.
...
2. **First publish of a never-published package.** A trusted publisher is configured on an existing package, so the very first publish of each package cannot use it. For that one run: create a granular access token on npmjs.com scoped to publish with two-factor bypass enabled (without the bypass the publish fails with `EOTP`, because CI cannot answer an OTP prompt), store it as a secret named `NPM_BOOTSTRAP_TOKEN` on the `npm-publish` environment (the publish job is that environment's only occupant, which scopes the secret more tightly than a repository secret; the workflow's `secrets.` lookup resolves either location), run the release workflow manually from the release tag with `dry-run` disabled and `bootstrap-token` enabled, then configure the trusted publisher as in step 1, **delete the secret, and revoke the token on npmjs.com**. The workflow exposes that secret to the two publish steps only, and only when the input is enabled; every later release must leave it disabled. This was performed for `v0.1.0` on 2026-08-27; both packages exist, the trusted publisher is configured on each, and no token secret remains.
...
## First release and the audited release

The first publish happened ahead of the audit: `v0.1.0` (2026-08-27) reserved both package names on npm via the bootstrap path in [One-time setup outside the repository](#one-time-setup-outside-the-repository), a manual run from the release tag with `bootstrap-token` enabled, followed immediately by configuring the trusted publisher on both packages and deleting the token. Because npm versions are immutable, `0.1.0` is permanently that pre-audit build: it is not the version the site pins or the Directory submission names.

The audited release is therefore `0.1.1` or later, and its ordering is fixed by the audit:
...
3. Bump the version everywhere it lives: both `package.json` files, `snap.manifest.json`, `DEFAULT_SNAP_VERSION` in `packages/connector/src/snap.ts`, and the connector test fixtures that assert the pinned version. Prerelease suffixes are refused across the pipeline by design (connector, site, and the release preflight all require a plain `X.Y.Z`).
4. Then follow the checklist above. The trusted publisher is configured on both packages, so the publish authenticates via OIDC and `bootstrap-token` stays disabled.
