...
- ⚠️ Build warning: `Math.random` detected in bundle — traced to `bignumber.js`'s `BigNumber.random()` method (transitive dep of stellar-sdk). We never call it; not used for key material. Document for audit; optionally strip via webpack config later.
