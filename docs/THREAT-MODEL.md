...
- **Buffered response bodies (accepted).** `readJsonBounded` checks the declared `Content-Length` before reading and the decoded length after, but the snaps `fetch` endowment offers no reliable streaming read, so a hostile endpoint omitting the header can buffer up to its full body in memory before the post-read cap throws. Impact is denial of service of the snap only, it requires a compromised hardcoded endpoint, and the abort timeout bounds how long the stream can run.
...
- **Cross-account linkability via enumeration (accepted).** `getAccounts` discloses all revealed addresses at once to a connected origin, which links them. That is inherent to an enumeration method: it requires a connection grant and returns only accounts the user explicitly added from the snap home page. A user who keeps separate accounts to avoid correlation should not use them with the same origin; the add-account confirmation dialog states this disclosure.
