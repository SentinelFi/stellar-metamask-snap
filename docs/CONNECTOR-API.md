...
Whether this snap is installed **at the pinned version**. For `npm:` IDs a version mismatch reads as not installed. Local development snaps carry no meaningful version and only need to be present. This is the discovery path and it answers a question rather than authorising anything: a true answer is never carried forward to satisfy a later call's version check, because the only thing that can be verified about a bundle is the version installed at the moment it is invoked.
...
The escape hatch for methods with no typed wrapper. Returns `unknown`: the response crossed the provider boundary and nothing has checked its shape. It shares the version check with the typed methods, so even the raw path cannot quietly talk to a release other than the pinned one. The method name plays no part in that check: a replacement snap installed under the same ID holds the same granted permissions and controls its own dispatcher, so it is not bound by the method a caller asked for, and no method name can make an invocation safe on the strength of an earlier comparison.

### Version mismatch

Every method that contacts the snap (the typed ones, `invoke()`, and through them the Freighter facade and the Wallets Kit module) throws a `StellarSnapError` with code `-3` when MetaMask reports a version of the snap other than the pinned one installed under the snap ID. The message names both versions. **Every** invocation re-reads `wallet_getSnaps` first, whatever the method: nothing is memoized between calls, because an earlier comparison describes the bundle installed then, and MetaMask can replace it under the same ID at any point in a page's life. Each call awaits a lookup begun after it was made rather than one already in flight, and a call whose own lookup is outdated by a later call's demand while it is pending repeats the check for that newer demand instead of proceeding on its older success, so a snap updated mid-session fails closed whichever call notices first. A snap that is not installed is refused here too, rather than deferred to MetaMask's own rejection: that deferral cannot support an exact-version guarantee during an install or update race, and `isInstalled()` remains the discovery path for callers that want it. The cost is one `wallet_getSnaps` read per invocation, which is the price of the guarantee. The check is repeated after any refusal (the user may update the snap between calls), and is skipped for `local:` IDs. A snap that is not installed at all is not a mismatch: MetaMask refuses the invocation itself, exactly as before, so pre-install handling is unchanged.
