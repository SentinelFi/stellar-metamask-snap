...
Whether this snap is installed **at the pinned version**. For `npm:` IDs a version mismatch reads as not installed. Local development snaps carry no meaningful version and only need to be present. A true answer also satisfies the per-call version check for the public network reads (`getNetwork`, `getNetworkDetails`), so asking this first costs one `wallet_getSnaps` read, not two; every other method, the address, account and balance reads included, still performs its own fresh comparison at call time.
...
The escape hatch for methods with no typed wrapper. Returns `unknown`: the response crossed the provider boundary and nothing has checked its shape. It shares the version check with the typed methods, so even the raw path cannot quietly talk to a release other than the pinned one. The method name is arbitrary here, so only the public-read allowlist (`getNetwork`, `getNetworkDetails`) may answer from the per-page memo; any other name, the signing methods and the connected privacy reads included, is compared against a fresh `wallet_getSnaps` read begun after the call was made and not outdated by a later one.

### Version mismatch

Every method that contacts the snap (the typed ones, `invoke()`, and through them the Freighter facade and the Wallets Kit module) throws a `StellarSnapError` with code `-3` when MetaMask reports a version of the snap other than the pinned one installed under the snap ID. The message names both versions. Only the public network reads (`getNetwork`, `getNetworkDetails`) remember a verified pin for the client instance: they disclose nothing about the wallet. Every other method re-reads `wallet_getSnaps` on every call: signing, dialog-confirmed mutations, `fund`, raw calls outside that allowlist, and the silent but wallet-specific reads `getAddress`, `getAccounts`, and `getBalances`, because a replacement snap installed under the same ID mid-session holds the same granted permissions and would otherwise run its code over the wallet's addresses, accounts, and balances on the strength of a memo written before the update. Each such call awaits a lookup begun after it was made rather than one already in flight, and a call whose own lookup is outdated by a later call's demand while it is pending repeats the check for that newer demand instead of proceeding on its older success, so a snap updated mid-session fails closed whichever call notices first. The check is repeated after any refusal (the user may update the snap between calls), and is skipped for `local:` IDs. A snap that is not installed at all is not a mismatch: MetaMask refuses the invocation itself, exactly as before, so pre-install handling is unchanged.
