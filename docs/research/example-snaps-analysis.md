...
### Dialogs
- **Strategy pattern per XRPL TransactionType** (`TransactionDialogStrategyFactory`): type-specific `buildBody`, header + origin, divider sections, raw-tx review footer, full i18n. Unsupported tx types throw — a *whitelist* of what can be signed.
...
- Pitfall found: `params.methods ?? []` — omitted methods default to *all-methods* access.
...
```javascript
const entropy = await snap.request({ method: 'snap_getEntropy', params: { version: 1, salt } });
